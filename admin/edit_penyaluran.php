...
$data = query("SELECT * FROM penyaluran where id_penerimaan = '$id'")[0];


if(isset($_POST['submit'])){
  if(editPenyaluran($_POST) > 0 ){
...
<body>
  <h2 class="text-center">Form edit Penyaluran</h2><hr width="800">
    <form id="form1" method="post" name="form1" action="" enctype="multipart/form-data">
    <input type="hidden" name="id" value="<?= $data['id_penerimaan'] ?>">
        <!--Untuk Nama lengkap -->
        <div class="mb-3 row">
    <label for="tgl_penerimaan" class="col-sm-2 col-form-label">Tanggal Penerimaan</label>
    <div class="col-sm-4">
      <input type="date" class="form-control" id="tgl_penerimaan" name=tgl_penerimaan required value="<?= htmlspecialchars($data['tgl_penerimaan']) ?>">
    </div>
  </div>
  <div class="mb-3">
    <label for="id_mustahik" class="form-label">Nama Penerima (Mustahik)</label>
    <select class="form-select" id="id_mustahik" name="nama_lengkap" required value="<?= $data['nama_penerima'] ?>">
    ...
    <div class="col-sm-4">
      <input type="text" class="form-control" id="uang" name=uang required value="<?= $data['jumlah_penerimaan']?>">
    </div>
  </div>

  <div class="mb-3">
    <label for="id_amil" class="col-sm-2 col-form-label">Nama Amil</label>
    <select class="form-select" id="id_amil" name="nama_amil" required value="<?= $data['amil']?>">
